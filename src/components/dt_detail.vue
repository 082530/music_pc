    ...
    <div class="song">
      <table>
        ...
        <tr v-for="(item,key) in list">
          <td>{{key+1}}</td>
          <td>{{item.name}}
            <a :href="'http://music.163.com/song/media/outer/url?id='+item.mainTrackId+'.mp3'">
            ...
            <a @click="addSong({id:item.mainTrackId,name:item.name});open()" title="加入播放列表">
          ...
          <td><router-link :to="{name:'songPlay',params:{id:item.mainTrackId}}">▶</router-link></td>
        </tr>
        </tbody>
      </table>
      <div class="page">
        <span @click="now_page=item;dt_list()" :class="item===now_page?'active':''" v-for="item in page_count">{{item}}</span>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted: function () {
    let that = this
    $.get(that.url + '/dj/detail?rid=' + that.id,
      function (data) {
        let info = data.djRadio
        that.author = info.rcmdText // 信息
        that.alb = info.name // 电台name
        that.pushlistTime = new Date(info.createTime).toLocaleDateString() // 时间
        that.description = info.desc // 简介
        that.img = info.picUrl + '?param=140y140'
        that.tags = info.category // 标签
      }
    )
    this.dt_list()
  }
